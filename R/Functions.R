...
aphid_loss <- function(rainfall){
  ...
  mod <- nls(Wl ~ SSlogis(Wr, Asym, xmid, scal), data = data.frame(Wr, Wl))
...
alate_production <- function(CP_fraction_aphid_capacity){
  if(CP_fraction_aphid_capacity <= 0){
    ...
    Af <- seq(0, 0.9, 0.1)
    App <- c(0.01, 0.22, 0.35, 0.45, 0.53, 0.6, 0.66, 0.72, 0.76, 0.78)
...
plant_infect <- function(Ph, Pi, Awt, Ai, Awtp, Aip, Ac, temperature, Aa, Aw){
...
pop_produce <- function(temperature, Ax, Ac, A, Apop){
